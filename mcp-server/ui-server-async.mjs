...
class TaskTracker {
  ...
  createTask(taskId, agentId, prompt) {
...
/**
 * MCP Server for Claude Code with Async Support
 * Provides both sync and async claude_code tools
 */
...
8. Claude can do much more, just ask it!`,
        ...
        },
        {
          ...
          description: `Async Claude Code: Execute Claude Code tasks asynchronously with callback notification.

• Executes Claude Code in the background
• Sends result back to the calling Letta agent via callback URL
• Useful for long-running tasks that might timeout in sync mode

**Required parameters:**
• prompt: The task to execute
• agentId: The Letta agent ID to notify when complete
• callbackUrl: URL to send the result to when task completes
...
**Response:**
{
  ...
  "message": "Task started successfully. Will notify agent_123 when complete."
          ...
          inputSchema: {
            ...
            properties: {
              ...
              agentId: {
                type: 'string',
                description: 'The agent ID to notify when the task completes.',
              },
              workFolder: {
                type: 'string',
              ...
              callbackUrl: {
                type: 'string',
                description: 'URL to POST the result to when task completes.',
              },
            },
            required: ['prompt', 'agentId', 'callbackUrl'],
          },
        }
      ],
    }));
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (args) => {
      ...
      if (name !== 'claude_code' && name !== 'claude_code_async') {
      ...
      // Handle async tool
      if (name === 'claude_code_async') {
        // Validate async-specific parameters
        if (!toolArgs.agentId || typeof toolArgs.agentId !== 'string') {
          throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: agentId');
        }
        if (!toolArgs.callbackUrl || typeof toolArgs.callbackUrl !== 'string') {
          throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: callbackUrl');
        }

        const taskId = `task_${randomUUID()}`;
        const { prompt, agentId, workFolder, callbackUrl } = toolArgs;

        // Store task info
        taskTracker.createTask(taskId, agentId, prompt);

        // Start async execution
        this.executeClaudeAsync(taskId, agentId, prompt, workFolder, callbackUrl).catch(error => {
  ...
  /**
   * Execute Claude Code through the UI backend (sync)
   */
   ...
   */
  async executeClaudeAsync(taskId, agentId, prompt, workFolder, callbackUrl) {
  ...
  async sendCallback(callbackUrl, data) {
    try {
      ...
      debugLog('Callback sent successfully:', data.taskId);
    } catch (error) {
